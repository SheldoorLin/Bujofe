...
class ReclassViewModel : ViewModel() {
    ...
    private val tagString: String = "ReclassViewModel"
...
//    val serviceTeachListInformation: LiveData<List<TeachList>>
...
//    private val teachListData = mutableListOf<TeachList>()
    ...
    private fun getUserDataFirebase() {
        FirebaseFirestore.getInstance().collection("users")
            .get()
            .addOnCompleteListener { task ->
                if (task.isSuccessful) {
                    for (document in task.result!!) {
                        Log.d(tagString, document.id + " => " + document.data)
                        val data = document.toObject(Users::class.java)
                        userData.add(data)
                        _serverUserInformation.value = userData
                        Log.d(tagString, "userData $userData")
                    }
                } else {
                    Log.w(tagString, "Error getting documents.", task.exception)
                }
            }
    }
...
//                        Logger.d(tagString + "teachListData $teachListData")
//                    }
//                } else {
//                    Log.w(tagString, "Error getting documents.", task.exception)
//                }
